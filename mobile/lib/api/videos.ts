...
export async function getVideos(filters?: {
  ...
  let query = supabase
    .from('videos')
    .select(`*, profiles!videos_teacher_id_fkey (name, avatar_url)`)
...
export async function getVideoStats(teacherId: string): Promise<VideoStats> {
  ...
  if (error || !videos) {
    return { totalVideos: 0, totalViews: 0, videosThisMonth: 0, studentCount: 0 };
  }
  ...
  const videosThisMonth = videos.filter((v: any) => new Date(v.created_at) >= startOfMonth).length;
  ...
  const studentCount = analytics
    ? new Set(analytics.map((a: any) => a.user_id).filter(Boolean)).size
    : 0;

  return { totalVideos, totalViews, videosThisMonth, studentCount };
}

export async function incrementVideoView(videoId: string, userId?: string, watchedDuration?: number): Promise<void> {
...
function mapDbVideoToVideo(dbVideo: any): Video {
  ...
  if (dbVideo.slides_data?.slides && Array.isArray(dbVideo.slides_data.slides)) {
